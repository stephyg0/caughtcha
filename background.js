...
const storageGet = (keys) =>
  new Promise((resolve) => chrome.storage.local.get(keys, (value) => resolve(value)));
...
function matchesBlockedUrl(urlText, hosts) {
  try {
    ...
    return hosts.some((entry) => {
      ...
      if (cleaned.includes("/")) return pathKey.startsWith(cleaned);
...
async function shouldAllowTab(tabId) {
...
async function getBlocklist() {
  const { customBlockedHosts } = await storageGet({ customBlockedHosts: DEFAULT_BLOCKED_HOSTS });
...
chrome.runtime.onInstalled.addListener(async () => {
  const existing = await storageGet("customBlockedHosts");
